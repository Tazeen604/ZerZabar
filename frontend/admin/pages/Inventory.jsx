...
const Inventory = () => {
  ...
  const filteredData = inventoryData.filter(item => {
    ...
    const lowStockThreshold = item.low_stock_threshold || 10; // Default to 10 if not set
    if (filterStatus === 'low') {
      matchesFilter = item.stock_quantity <= lowStockThreshold && item.stock_quantity > 0;
  ...
  const getTotalValue = () => {
    return inventoryData.reduce((total, item) => {
      return total + (item.stock_quantity * item.price || 0);
    }, 0);
  };

  const getTotalItems = () => {
    return inventoryData.reduce((total, item) => total + item.stock_quantity, 0);
  };
            ...
            <TableBody>
              {filteredData
                ...
                .map((item) => {
                  ...
                  const totalValue = item.stock_quantity * (item.price || 0);
                  ...
                  return (
                    <TableRow key={item.id} hover>
                      ...
                      <TableCell>PKR {item.price || 0}</TableCell>
                      <TableCell>
                        <Typography variant="body2" sx={{ fontWeight: 'bold' }}>
