...
const ProductView = () => {
  ...
  useEffect(() => {
    if (product?.images?.length) {
      // Fix: Use exact image count * viewport height for proper container height
      const viewportHeight = window.innerHeight;
      const totalImages = product.images.length;
      const totalHeightValue = totalImages * viewportHeight;
      setTotalHeight(totalHeightValue);
      setCurrentIndex(0); // Reset to first image when product changes
      
      console.log('SETUP: images:', product.images.length, '| totalHeight:', totalHeightValue, '| viewportHeight:', window.innerHeight, '| calculated:', totalImages * window.innerHeight);
    }
  }, [product?.images]);

  // Enhanced scroll handler with proper timing and sticky behavior
  useEffect(() => {
    const onScroll = () => {
      if (!containerRef.current || !innerRef.current || !product?.images) return;
      ...
      // Fix: Use full viewport height for threshold (not 30%)
      const threshold = viewportHeight; // Change index when fully scrolled through image
      
      // Slide images - stop when last image is fully visible
      const maxImageTranslation = (product.images.length - 1) * viewportHeight;
      ...
      // Fix: Calculate image index based on full viewport height
      const imageIndex = Math.floor(scrollInContainer / viewportHeight);
      const index = Math.min(product.images.length - 1, Math.max(0, imageIndex));
      
      // Fix: Show image index only during image transitions, hide when last image starts
      const lastImageStart = (product.images.length - 1) * viewportHeight;
      const shouldShowIndex = scrollInContainer < lastImageStart;
      setShowImageIndex(shouldShowIndex);
      
      // Let CSS sticky handle the right panel behavior naturally
      console.log('SCROLL: scrollInContainer:', scrollInContainer, '| index:', index, '| shouldShowIndex:', shouldShowIndex);
    ...
    <Box sx={{ backgroundColor: "#fff", pt: "100px" }}>
      ...
      <Box sx={{ 
        ...
        <Box sx={{ 
          ...
          <Box ref={containerRef} sx={{ 
            position: "relative", 
            height: totalHeight > 0 ? `${totalHeight}px` : '100vh', 
            overflow: "hidden",
            borderRadius: 2
          }}>
            <Box ref={innerRef} sx={{ 
      ...
      <Box sx={{ display: { xs: 'block', md: 'none' }, maxWidth: 1200, mx: 'auto', p: 2, mt: 2 }}>
       ...
       <Box sx={{ maxWidth: 1200, mx: 'auto', p: 3, mt: 4 }}>
         ...
         <Box sx={{ 
           display: 'grid',
           gridTemplateColumns: { xs: 'repeat(auto-fit, minmax(200px, 1fr))', sm: 'repeat(4, 1fr)' },
           gap: 2,
           pb: 2
         }}>
           {/* Related Product 1 */}
